...
if __name__ == "__main__":
    ...
    video_save_path = "rtout3.mp4"#"user_files"
    video_fps       = 25.0
    #----------------------------------------------------------------------------------------------------------#
...
#     )    
    st.sidebar.markdown(
        '<a ' + authors_css + ' target="_blank" href="http://ies.ncu.edu.cn/">' + '江西省大学生人工智能科技大赛</a>',
        unsafe_allow_html=True,
    )        
    if choose == "拍照识鸟":
        #-------------------------------------------------------------------------#
        ...
        st.info('为了处理突发性输电线路涉鸟故障，针对性地加装防鸟措施，:balloon:本软件可为您识别涉鸟故障危害鸟种，以便为运维人员提供正确识鸟的工具。(请上传jpg格式的图片)') 
        #-------------------------------------------------------------------------#
        # 图片文件加载处
        #-------------------------------------------------------------------------#        
        img = st.file_uploader("Choose a file（请选择图片格式文件）", key = "<uniquevalueofsomesort>", type=['jpg']) 
        #-------------------------------------------------------------------------# 
        # ？
        #-------------------------------------------------------------------------#        
        ...
        st.info('为了处理突发性输电线路涉鸟故障，针对性地加装防鸟措施，:baby_chick:本软件可为您识别涉鸟故障危害鸟种，以便为运维人员提供正确识鸟的工具。(请上传mp4格式的视频)') 
        #-------------------------------------------------------------------------#
        # 进度条
        #-------------------------------------------------------------------------# 
        ...
        if video_path:
            ...
            while(capture.isOpened()):
                ...
                if frame_counter == frame_frequency:                # 表示每 frame_frequency帧检测一次，后期可将其调整为用户可调节参数（可推拉进度条等）
                    ...
                    # 进行检测
                    frame = np.array(yolo.detect_image(frame, False, False, detection_result))
