...
class YOLO(object):
    ...
    def detect_image(self, image, crop = False, count = False, result_t={}):
        ...
        result_table.iloc[0,0] = f'<a href="{link_Baidu}" target="_blank">{Chi_EngName[Predicted_LableClass]}</a>'   #标签->中文名
        # 显示识别故障鸟种置信度
        result_table.iloc[0, 1] = top_conf[top_label_index[0]]
    ...
    def detect_heatmap(self, image, heatmap_save_path):
        ...
        def sigmoid(x):
            ...
            return y
        #---------------------------------------------------------#
        #   在这里将图像转换成 RGB图像，防止灰度图在预测时报错。
        #   代码仅仅支持 RGB图像的预测，所有其它类型的图像都会转化成RGB
        #---------------------------------------------------------#
        image       = cvtColor(image)
        #---------------------------------------------------------#
